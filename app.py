...
VERSION = "3.0-binance-crypto"   # ← برای تشخیص اینکه کدام نسخه روی Render هست
...
def _crypto_binance():
    """قیمت بیت‌کوین و تتر از Binance - معمولاً پایدارتر از CoinGecko روی سرورهای ابری"""
    btc, usdt = None, None
    try:
    ...
    try:
        r = requests.get("https://api.binance.com/api/v3/ticker/price?symbol=USDCUSDT", timeout=10)
        if r.status_code == 200:
            usdc_per_usdt = float(r.json()["price"])
            if usdc_per_usdt > 0:
...
def _crypto_coingecko():
    try:
        ...
        if r.status_code == 200:
            ...
            if btc:
                print(f"[coingecko] BTC=${btc:,.2f}")
            return btc, usdt
        print(f"[coingecko] {r.status_code}")
    except Exception as e:
        print(f"[coingecko] خطا: {e}")
    return None, None

def get_crypto_prices_usd():
    print("[crypto] شروع دریافت قیمت بیت‌کوین/تتر...")
    btc, usdt = _crypto_binance()
    if not btc:
        btc2, usdt2 = _crypto_coingecko()
        btc = btc or btc2
        usdt = usdt or usdt2
    if not usdt:
        usdt = 1.0  # تتر معمولاً خیلی نزدیک به ۱ دلاره؛ fallback امن
    if not btc:
        print("[crypto] ❌ هیچ منبعی برای بیت‌کوین کار نکرد")
    return btc, usdt
